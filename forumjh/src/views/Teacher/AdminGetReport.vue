<template>
    ...
    <div class="wrapper">
        ...
        <nav>
        <RouterLink :to="{ name: 'teachergetreport', query : route.query}">查看未审批举报</RouterLink>
        <RouterLink :to="{ name: 'teacherproreport', query : route.query}">举报处理</RouterLink>
        </nav>
      </div>
    </header>
      ...
      <div class="post-list-wrapper">
        <div class="post-list">
          ...
          <div v-for="post in posts" :key="post.id" class="post">
            <div class="user-info">
              帖子编号：{{ post.PostID }}  |  举报的用户名: {{post.Username}}
            </div>
            <div class="content">
                举报内容：
              {{ post.Content }}
...
<script setup>
  ...
  onMounted(async () => {
    try {
      const response = await axios.get('http://127.0.0.1:8080/api/admin/report', {
